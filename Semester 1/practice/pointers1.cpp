...
int main(){
    ...
    for(int i=0 ; i<3 ; i++){
        ...
        int *ptr1 = arr + index;  //pointer to the maximum element found
        cout<<"Maximum "<<i+1<<" : "<<max<<" at index "<<index<<"(Address : "<<ptr1<<" )"<<endl;  //printing the maximum value
        *(arr + index) = -99999;   //setting the found maximum to a very small value to find next maximum in next iteration
    }
